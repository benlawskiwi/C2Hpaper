...
for i in range(0,np.size(lab)):
    if i == 1: continue
    if i ==0:
        plt.annotate(lab[i],(xx[i]-100,max(ym[i],by[i])+0.03),ha='center',color=col[i])
        continue
    if i ==14:
        plt.annotate(lab[i],(xx[i]+100,max(ym[i],by[i])+0.03),ha='center',color=col[i])
        continue
    plt.annotate(lab[i],(xx[i],max(ym[i],by[i])+0.03),ha='center',color=col[i])
...
ax.set_xticklabels([24000,26000,28000,30000],fontsize=14)
