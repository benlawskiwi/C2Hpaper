...
#Ground X
popt, pcov = curve_fit(bet,dex,dbx,sigma=ddbx)
perr = np.sqrt(np.diag(pcov))
print ('A=',popt[0],'+/-',perr[0])
print ('f=',popt[1],'+/-',perr[1])

#Ground HT
popt2, pcov2 = curve_fit(bet,ce,cb,sigma=cx)
perr2 = np.sqrt(np.diag(pcov2))
print ('A=',popt2[0],'+/-',perr2[0])
print ('f=',popt2[1],'+/-',perr2[1])

#Excited A
popt3, pcov3 = curve_fit(bet,dea,dba,sigma=ddba)
perr3 = np.sqrt(np.diag(pcov3))
print ('A=',popt3[0],'+/-',perr3[0])
print ('f=',popt3[1],'+/-',perr3[1])
...
plt.plot(grid,bet(grid,popt2[0],popt2[1]),'C0--', lw=2)#,label='89% p')
#plt.plot(grid,bet(grid,popt3[0],popt3[1]),'C0--', lw=2,label='p (f=0.9)')

plt.errorbar(dex,dbx,yerr=ddbx,fmt='o',markersize=8,color='C1',label=r'C$_2$D $^2\Sigma^+$')
plt.errorbar(dexh,dbxh,yerr=ddbxh,fmt='o',markersize=8,color='C1',mfc='white',label=r'C$_2$D $^2\Sigma^+$-HT')
plt.errorbar(dea,dba,yerr=ddba,fmt='o',markersize=8,color='C0',mfc='white',label=r'C$_2$D $^2\Pi$')
...
ax.legend(loc='lower center', fontsize=11, ncol=3,frameon=True,bbox_to_anchor=(0,-0.01,1,-0.01))
plt.xlim(0,0.6)

plt.savefig("D-PAD.pdf", dpi=400, bbbox_inches="tight")
#plt.legend()
plt.show()
